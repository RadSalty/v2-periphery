...
import './interfaces/IPxlswapV2Router02.sol';
import './libraries/PxlswapV2Library.sol';
import './libraries/SafeMath.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract PxlswapV2Router02 is IPxlswapV2Router02 {
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = PxlswapV2Library.sortTokens(input, output);
            IPxlswapV2Pair pair = IPxlswapV2Pair(PxlswapV2Library.pairFor(factory, input, output));
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = PxlswapV2Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint256 amount0Out, uint256 amount1Out) = input == token0
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'PxlswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(PxlswapV2Library.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, 'PxlswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function getAmountIn(
        ...
        return PxlswapV2Library.getAmountIn(amountOut, reserveIn, reserveOut);
    }
